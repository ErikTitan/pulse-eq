...
<template>
  <div class="audio-upload-manager">
    <div
      class="border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300 ease-in-out group"
      :class="{
        'border-primary-500 bg-primary-50/50 dark:bg-primary-900/20': dragActive,
        'border-surface-200 dark:border-surface-700 hover:border-primary-300 dark:hover:border-primary-700 hover:bg-surface-50 dark:hover:bg-surface-800':
          !dragActive,
      }"
      @drop="handleDrop"
      @dragover.prevent
    ...
    <div v-if="uploadedFiles.length > 0" class="mt-6">
      <div
        class="text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider mb-3 px-1"
      >
        Uploaded Files
      </div>
      <div class="space-y-2">
        ...
        >
          <div class="flex items-center space-x-3 overflow-hidden">
            ...
            >
              <i
                :class="[getIconClass(file), 'text-primary-600 dark:text-primary-400 text-sm']"
              ></i>
            </div>
            <div class="min-w-0">
              <p
                ...
                :class="{
                  'text-primary-600 dark:text-primary-400':
                    audioUploadStore.selectedAudioSource === file.id,
                }"
              >
                {{ file.name }}
              </p>
              <p class="text-xs text-surface-500 dark:text-surface-400">
                {{ Math.round(file.duration) }}s • {{ (file.size / 1024 / 1024).toFixed(1) }}MB
              </p>
            </div>
          </div>
          <Button
