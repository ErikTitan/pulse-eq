...
export default {
  ...
  methods: {
    drawFrequencyResponse() {
      ...
      const lineColor = style.getPropertyValue('--response-line-color').trim()
      ...
      for (let i = 0; i < frequencies.length; i++) {
        ...
        frequencies[i] = Math.pow(
          10,
          Math.log10(20) + logScale * (Math.log10(this.nyquist) - Math.log10(20)),
        )
      }
      ...
      this.filters.forEach((filter, index) => {
        if (!filter.bypass && filter.type !== 'noop') {
          this.weq8.getFrequencyResponse(index, 0, frequencies, magResponse, phaseResponse)
      ...
      // Draw response curve
      ctx.beginPath()
      ctx.strokeStyle = lineColor || '#ffffff'
      ctx.lineWidth = 2
