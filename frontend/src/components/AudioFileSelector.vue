...
<template>
    <div class="audio-file-selector">
        <label for="audio-source-select" class="block text-sm font-medium mb-2 text-surface-700">
            Audio Source
        </label>
        <Select id="audio-source-select" v-model="selectedAudioSource" :options="audioSources" option-label="name"
            option-value="id" placeholder="Select audio source" class="w-full" :class="{
                'ring-2 ring-primary-200': hasUploadedFiles,
            }">
            <template #option="slotProps">
                <div class="flex items-center justify-between w-full">
                    <div class="flex items-center space-x-2">
                        <i :class="slotProps.option.isDefault ? 'pi pi-music' : 'pi pi-file-audio'" class="text-sm"
                            :style="{ color: slotProps.option.isDefault ? 'var(--p-primary-500)' : 'var(--p-surface-500)' }"></i>
            ...
            <template #value="slotProps">
                <div v-if="slotProps.value" class="flex items-center space-x-2">
                    <i :class="audioSources.find(s => s.id === slotProps.value)?.isDefault ? 'pi pi-music' : 'pi pi-file-audio'"
                        class="text-sm" :style="{
        ...
        <div class="mt-1 text-xs text-surface-500">
            <span v-if="!hasUploadedFiles">
                Upload audio files to see more options
            </span>
            <span v-else>
                {{ audioSources.length - 1 }} uploaded file{{ audioSources.length - 1 !== 1 ? 's' : '' }} available
            </span>
        </div>
    </div>
</template>
...
<style scoped>
/* Additional styling using PrimeVue tokens and Tailwind CSS */
.audio-file-selector {
...
/* Subtle animation for the ring effect when files are available */
.audio-file-selector :deep(.p-select.ring-2) {
    transition: box-shadow 0.2s ease-in-out;
}

/* Custom styling for the dropdown options */
.audio-file-selector :deep(.p-select-option) {
    padding: 0.75rem;
}

/* Ensure proper spacing in the option template */
