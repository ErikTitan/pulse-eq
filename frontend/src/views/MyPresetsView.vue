<template>
  <div class="min-h-screen flex-1 pt-24 px-6 lg:px-20">
    ...
    <Dialog
      ...
      :header="editingPreset ? 'Edit Preset' : 'Create Preset'"
      :modal="true"
    ...
    >
      <form @submit.prevent="savePreset" class="space-y-4">
        ...
        <div>
          ...
          <Select
            id="category"
            v-model="presetForm.preset_category_id"
        ...
        <div>
          ...
          <small class="text-xs text-gray-400"
            >{{ (presetForm.description || '').length }}/80 characters</small
          >
        </div>
        <div>
          <label for="settings" class="block text-sm font-medium text-gray-300"
            >Settings (JSON)</label
          >
        ...
        <div>
          ...
          <InputText
            ...
            placeholder="Enter tags separated by commas (e.g. bass, gaming, vocal)"
            class="w-full"
          />
        ...
        <div class="flex justify-end gap-2">
          ...
          <Button
            type="submit"
            :label="editingPreset ? 'Update' : 'Create'"
    ...
    >
      <form @submit.prevent="updateProfile" class="space-y-4">
        <div class="flex justify-center">
          <UserAvatar
            :user="authStore.user"
            :label="authStore.avatarLabel"
            :variant="authStore.avatarVariant"
            size="xlarge"
          />
        </div>
        <div>
  ...
  methods: {
    ...
    editPreset(preset) {
      ...
      try {
        ...
        console.error(
          `Could not parse settings for preset ${preset.id} on edit:`,
          preset.settings,
          e,
        )
      }
    ...
    sharePreset(preset) {
      if (!preset.public) {
        this.$toast.add({
          ...
          detail: 'This preset is private. Make it public to share it.',
          life: 3000,
        })
        return
      }
